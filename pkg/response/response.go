...
// Response 响应处理
//
//	有多处响应时，直接调用 JSON 函数，将会多处转入 ctx，使用结构体只需要转入一次
...
func (class *Response) JSON(data any) {
	JSON(class.ctx, data)
}

func (class *Response) Success() {
	Success(class.ctx)
}

func (class *Response) Data(data any) {
	Data(class.ctx, data)
}

func (class *Response) Created(data any) {
	Created(class.ctx, data)
}

func (class *Response) CreatedJSON(data any) {
	CreatedJSON(class.ctx, data)
}

func (class *Response) Abort404(msg ...string) {
	Abort404(class.ctx, msg...)
}

func (class *Response) Abort403(msg ...string) {
	Abort403(class.ctx, msg...)
}

func (class *Response) Abort500(msg ...string) {
	Abort500(class.ctx, msg...)
}

func (class *Response) Unauthorized(msg ...string) {
	Unauthorized(class.ctx, msg...)
}

func (class *Response) BadRequest(err error, msg ...string) {
	BadRequest(class.ctx, err, msg...)
}

func (class *Response) Error(err error, msg ...string) {
	Error(class.ctx, err, msg...)
}

func (class *Response) ValidationError(errors map[string][]string) {
	ValidationError(class.ctx, errors)
}
